...
def wdc_datetimes(data):
    ...
    data.drop(['year', 'yr', 'century', 'code', 'day', 'month'], axis=1, inplace=True)
...
def daily_mean_conversion(df):
    ...
    for group in df.groupby('component'):
        ...
        if group[0] == 'D' or group[0] == 'I':
            group[1]['daily_mean'] = group[1]['base'] + (1/600.0)*group[1]['daily_mean_temp']
            grp = grp.append(group[1],ignore_index=True)
        else:
            group[1]['daily_mean'] = 100.0*group[1]['base'] + group[1]['daily_mean_temp']
            grp = grp.append(group[1],ignore_index=True)
    return grp       
